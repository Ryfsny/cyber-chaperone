...
function mainMenuText(name: string, member: MemberInfo | null): string {
  ...
  if (isOperator) {
    return [
      ...
      ``,
      `1️⃣  Cyber Chaperone 🛡️ — trip monitor & Situation Room line`,
...
function ccMenuText(name: string): string {
  return [
    ...
    ``,
    `Your direct line to the eblockwatch Situation Room and André Snyman.`,
    `Use it to monitor your trips — and to communicate with us any time.`,
    ``,
    `1️⃣  Start a monitored trip 🗺️`,
    `2️⃣  Update my current trip`,
    `3️⃣  Change my destination`,
    `4️⃣  I have arrived safely ✅`,
    `5️⃣  I need help 🆘`,
    `6️⃣  How Cyber Chaperone works`,
    `7️⃣  Speak to André directly`,
    ``,
    `Reply 0 for Main Menu.`,
  ].join("\n");
}
...
function ccInfoText(name: string): string {
  return [
    `${name}, here is what Cyber Chaperone is:`,
    ``,
    `*It is two things in one:*`,
    ``,
    `🛡️ *A trip monitor*`,
    `   You tell us where you're going and when you'll arrive.`,
    `   We watch your route. If you don't check in — we act.`,
    `   Your ICE contact is alerted if we cannot reach you.`,
    ``,
    `📡 *A direct communication channel*`,
    `   This WhatsApp chat connects you to the eblockwatch`,
    `   Situation Room and André Snyman — any time, any reason.`,
    `   You are never alone. A real person is always on the other end.`,
    ``,
    `─────────────────────`,
    `🟢 Start — send your location and destination.`,
    `🔵 Monitor — we watch. No news from you = we check in.`,
    `📍 Check-ins — gentle prompts along the way.`,
    `✅ Arrive — reply SAFE and your trip is closed.`,
    `🆘 Help — reply HELP or 10 and we escalate immediately.`,
    `─────────────────────`,
    ``,
    `André is watching. You are not alone on the road.`,
    ``,
    `Reply 0 for Main Menu.`,
  ].join("\n");
}
